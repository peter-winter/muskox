...
TEST_CASE("nullable computation", "[nullable]")
{
    ...
    SECTION("no nullables")
    {
        ...
        REQUIRE_FALSE(rs.is_rside_part_nullable(root_idx, 0, 0));
        ...
        // S -> a (r=0, scount=1)
        REQUIRE_FALSE(rs.is_rside_part_nullable(s_idx, s_r0, 0));
    }
    ...
    SECTION("direct empty production")
    {
        ...
        // $root -> S (r=0, scount=1)
        REQUIRE(rs.is_rside_part_nullable(root_idx, 0, 0));
    }
    ...
    SECTION("indirect nullable through chain")
    {
        ...
        // S -> C (r=0, scount=1)
        REQUIRE(rs.is_rside_part_nullable(s_idx, s_r0, 0));

        // B -> A (r=0, scount=1)
        REQUIRE(rs.is_rside_part_nullable(b_idx, b_r0, 0));

        // C -> B A (r=0, scount=2)
        REQUIRE(rs.is_rside_part_nullable(c_idx, c_r0, 0));
        REQUIRE(rs.is_rside_part_nullable(c_idx, c_r0, 1));
    }
    ...
    SECTION("partial nullables")
    {
        ...
        REQUIRE_FALSE(rs.is_rside_part_nullable(s_idx, s_r0, 0));
        REQUIRE_FALSE(rs.is_rside_part_nullable(s_idx, s_r0, 1));
        REQUIRE(rs.is_rside_part_nullable(s_idx, s_r0, 2));
    }
    ...
    SECTION("multiple productions, some nullable")
    {
        ...
        // S -> b (r=1, scount=1)
        REQUIRE_FALSE(rs.is_rside_part_nullable(s_idx, s_r1, 0));

        // A -> b (r=1, scount=1)
        REQUIRE_FALSE(rs.is_rside_part_nullable(a_idx, a_r1, 0));
    }
    ...
    SECTION("suffixes with mixed terms and nullables")
    {
        ...
        REQUIRE_FALSE(rs.is_rside_part_nullable(s_idx, s_r0, 0));
        REQUIRE_FALSE(rs.is_rside_part_nullable(s_idx, s_r0, 1));
        REQUIRE_FALSE(rs.is_rside_part_nullable(s_idx, s_r0, 2));
        REQUIRE_FALSE(rs.is_rside_part_nullable(s_idx, s_r0, 3));
    }
    ...
    SECTION("suffixes with trailing nullables")
    {
        ...
        // S -> x A B (scount=3)
        REQUIRE_FALSE(rs.is_rside_part_nullable(s_idx, s_r0, 0));
        REQUIRE(rs.is_rside_part_nullable(s_idx, s_r0, 1));
        REQUIRE(rs.is_rside_part_nullable(s_idx, s_r0, 2));
    }
    ...
    SECTION("complex grammar with varying lengths")
    {
        ...
        REQUIRE_FALSE(rs.is_rside_part_nullable(expr_idx, expr_r0, 0));
        REQUIRE_FALSE(rs.is_rside_part_nullable(expr_idx, expr_r0, 1));
        REQUIRE(rs.is_rside_part_nullable(expr_idx, expr_r0, 2));

        // Expr -> Term (r=1, scount=1)
        REQUIRE(rs.is_rside_part_nullable(expr_idx, expr_r1, 0));

        // Term -> id (r=0, scount=1)
        REQUIRE_FALSE(rs.is_rside_part_nullable(term_idx, term_r0, 0));
    }
    ...
    SECTION("invalid indices in is_rside_part_nullable")
    {
        [[maybe_unused]] size_t s_idx = sc.add_nterm("S");
        [[maybe_unused]] size_t a_idx = sc.add_term("a");
        sc.validate();
        ...
        REQUIRE_THROWS_MATCHES(
            rs.is_rside_part_nullable(rs.get_nterm_count(), 0, 0),
            std::out_of_range,
            Message("Nterm index out of range")
        );

        REQUIRE_THROWS_MATCHES(
            rs.is_rside_part_nullable(0, rs.get_nterm_rside_count(0), 0),
            std::out_of_range,
            Message("Rside index out of range")
        );

        REQUIRE_THROWS_MATCHES(
            rs.is_rside_part_nullable(0, 0, rs.get_symbol_count(0, 0)),
            std::out_of_range,
    ...
    SECTION("individual calculate_rside_part - nullable")
    {
        ...
        REQUIRE(rs.is_rside_part_nullable(s_idx, s_r0, 0) == true);
        REQUIRE(rs.is_rside_part_nullable(s_idx, s_r0, 1) == true);
    }
    ...
    SECTION("individual calculate_rside_part - not nullable")
    {
        ...
        REQUIRE(rs.is_rside_part_nullable(s_idx, s_r0, 0) == false);
        REQUIRE(rs.is_rside_part_nullable(s_idx, s_r0, 1) == false);
    }
