...
TEST_CASE("closure multiple lookaheads", "[closure]")
{
    symbol_collection ml_sc;
    [[maybe_unused]] size_t ml_d_idx = ml_sc.add_term("d");
    [[maybe_unused]] size_t ml_e_idx = ml_sc.add_term("e");
    [[maybe_unused]] size_t ml_f_idx = ml_sc.add_term("f");
    [[maybe_unused]] size_t ml_g_idx = ml_sc.add_term("g");
    [[maybe_unused]] size_t ml_eof_idx = 0;  // $eof

    [[maybe_unused]] size_t ml_root_idx = 0;  // $root
    [[maybe_unused]] size_t ml_s_idx = ml_sc.add_nterm("S");
    [[maybe_unused]] size_t ml_n_idx = ml_sc.add_nterm("N");
    [[maybe_unused]] size_t ml_c_idx = ml_sc.add_nterm("C");
    ml_sc.validate();

    ruleset ml_rs(ml_sc);
    [[maybe_unused]] size_t ml_s_r0 = ml_rs.add_rule("S", {"N", "C", "d"});
    [[maybe_unused]] size_t ml_n_r0 = ml_rs.add_rule("N", {"g"});
    [[maybe_unused]] size_t ml_c_r0 = ml_rs.add_rule("C", {"e"});
    [[maybe_unused]] size_t ml_c_r1 = ml_rs.add_rule("C", {"f"});
    [[maybe_unused]] size_t ml_c_r2 = ml_rs.add_rule("C", {});
    ml_rs.validate();

    closure ml_cl(ml_rs);

    lr1_set_item ml_item(ml_s_idx, ml_s_r0, 0, ml_eof_idx);  // S -> . N C d / $eof
    const auto& ml_res = ml_cl.calculate_full(ml_item);
    REQUIRE(ml_res.has_value());
    REQUIRE(ml_res.value().get_count() == 4);
    REQUIRE(ml_res.value().contains(ml_s_idx, ml_s_r0, 0, ml_eof_idx));
    REQUIRE(ml_res.value().contains(ml_n_idx, ml_n_r0, 0, ml_d_idx));
    REQUIRE(ml_res.value().contains(ml_n_idx, ml_n_r0, 0, ml_e_idx));
    REQUIRE(ml_res.value().contains(ml_n_idx, ml_n_r0, 0, ml_f_idx));
}

TEST_CASE("closure chained nonterminals", "[closure]")
{
...
TEST_CASE("closure chained nonterminals with nullable", "[closure]")
{
    symbol_collection chain_sc;
    [[maybe_unused]] size_t chain_c_idx = chain_sc.add_term("c");
    [[maybe_unused]] size_t chain_eof_idx = 0;
...
TEST_CASE("closure chained nonterminals with nullable suffix", "[closure]")
{
    symbol_collection chain_sc;
