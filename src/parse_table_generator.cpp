...
void parse_table_generator::generate_states()
{
    ...
    lr1_state root_state(rs_, std::move(root_kernel));
    ...
    for (size_t i = 0; i < states_.size(); ++i)
    {
        ...
        for (const auto& [ref, act] : actions)
        {
            if (std::holds_alternative<lr1_state::shift>(act))
            {
                ...
                process_shift(ref.index_, s);                
            }
            else if (std::holds_alternative<lr1_state::reduction>(act))
            {
                const auto& r = std::get<lr1_state::reduction>(act);
                process_reduce(ref.index_, r);
            }
            else
            {
                const auto& c = std::get<lr1_state::conflict>(act);
                process_conflict(ref.index_, c);
            }
        }
    }
}

void parse_table_generator::process_shift(size_t /*term_idx*/, const lr1_state::shift& s)
{
...
void parse_table_generator::process_reduce(size_t /*term_idx*/, const lr1_state::reduction& /*r*/)
{
}

void parse_table_generator::process_conflict(size_t term_idx, const lr1_state::conflict& c)
{
    const lr1_state::reduction& red = c.r_[0]; // ignore more than one reduction for now
    
    if (c.s_.has_value())
    {
        auto s_prec = rs_.get_term_prec(term_idx);
        size_t r_prec = rs_.calculate_rside_precedence(red.nterm_idx_, red.rside_idx_);
        if (s_prec.has_value() && s_prec.value() > r_prec)
        {
            process_shift(term_idx, c.s_.value());
            return;
        }
    }
    // rr conflict
    process_reduce(term_idx, red);
}
